...
def sync_data():
    ...
    # --- NEW: Delete missing/removed posts from Database ---
    fetched_wp_ids = [post['ID'] for post in posts]
    if fetched_wp_ids:
        cursor.execute("SELECT wp_id FROM homedata")
        db_ids = [row['wp_id'] for row in cursor.fetchall()]
        for db_id in db_ids:
            if db_id not in fetched_wp_ids:
                print(f"Deleting post {db_id} as it was removed from WordPress.")
                cursor.execute("DELETE FROM homedata WHERE wp_id = %s", (db_id,))
        conn.commit()

    # --- Insert / Update valid posts ---
    for post in posts:
        wp_id = post['ID']
        title = post['title']
        ...
        if existing_post:
            # Puraane broken posts ko recover karna
