...
# --- IMAGE PROCESSING (Robust WebP & SVG Handler) ---
def create_resized_webp(img, target_kb, max_w, max_h):
    """Helper function to resize and dynamically compress to target KB"""
...
def compress_image(image_url, wp_id):
    ...
    try:
        ...
        # 1. Handle SVG images gracefully (Do not compress, save as-is)
        if image_bytes.strip().startswith(b'<svg') or image_bytes.strip().startswith(b'<?xml'):
            feat_filename = f"feat_{wp_id}.svg"
            grid_filename = f"grid_{wp_id}.svg"
            # Write to cache immediately
            with open(os.path.join(IMAGE_DIR, feat_filename), "wb") as f: f.write(image_bytes)
            with open(os.path.join(IMAGE_DIR, grid_filename), "wb") as f: f.write(image_bytes)
            return feat_filename, grid_filename, image_bytes, image_bytes

        img = Image.open(BytesIO(image_bytes))
        
        # 2. Proper Transparency Fix: Replace transparent bg with White
        ...
        # 3. Compress for Slider (30KB) and Grid (10KB)
        feat_bytes = create_resized_webp(img.copy(), 30, 800, 450)
        grid_bytes = create_resized_webp(img.copy(), 10, 300, 300)
        
        feat_filename = f"feat_{wp_id}.webp"
        grid_filename = f"grid_{wp_id}.webp"

        # Write to local cache directly so first load is fast
        with open(os.path.join(IMAGE_DIR, feat_filename), "wb") as f: f.write(feat_bytes)
        with open(os.path.join(IMAGE_DIR, grid_filename), "wb") as f: f.write(grid_bytes)

        return feat_filename, grid_filename, feat_bytes, grid_bytes
        
    except Exception as e:
        print(f"Image compression skipped (format unsupported/Corrupt): {e}")
...
def sync_data():
    ...
    # 1. Fetch APPROVED list from your Backend API
    try:
        mapping_res = requests.get(ALLPOST_API_URL, timeout=15)
        mapping_res.raise_for_status()
        allpost_data = mapping_res.json()
        
        # Dictionary of approved URLs: { "wp_original_url": "custom_slug" }
        approved_mappings = {unquote(item['original_url']): item['slug'] for item in allpost_data}
    except Exception as e:
        print(f"Backend API Error (Skipping sync to prevent accidental deletions): {e}")
        return # Agar backend down hai toh ruk jao, kuch delete mat karo

    conn = get_db_connection()
    cursor = conn.cursor()

    # 2. DELETE unapproved/removed posts from Homepage Database
    cursor.execute("SELECT wp_id, original_url FROM homedata")
    db_posts = cursor.fetchall()
    
    for db_post in db_posts:
        db_url = unquote(db_post['original_url'])
        if db_url not in approved_mappings:
            print(f"Deleting post {db_post['wp_id']} because it was removed from backend approval list.")
            cursor.execute("DELETE FROM homedata WHERE wp_id = %s", (db_post['wp_id'],))
    conn.commit()

    # 3. Fetch latest posts from WordPress (Fetch 100 to be safe)
    try:
        # number=100 ensures we don't miss any recently approved posts
        wp_res = requests.get("https://public-api.wordpress.com/rest/v1.1/sites/pranavcea.wordpress.com/posts/?number=100", timeout=15)
        wp_posts = wp_res.json().get('posts', [])
    except Exception as e:
        print(f"Failed to fetch WP posts: {e}")
        wp_posts = []

    # 4. ADD / UPDATE only those posts which are APPROVED
    for post in wp_posts:
        original_url = unquote(post['URL'])
        
        # CRITICAL RULE: Agar WordPress ka ye link tumhare /allpost me nahi hai, toh chhod do! (Skip)
        if original_url not in approved_mappings:
            continue 

        wp_id = post['ID']
        title = post['title']
        slug = approved_mappings[original_url] # Apne backend wala slug use karo
        date = post['date']
        ...
        # Naya APPROVED post mila, isko compress karke database me daalo
        f_name, g_name, f_bytes, g_bytes = compress_image(image_url, wp_id)
...
def generate_static_html():
    ...
    # [NEW] Google AdSense Script for Verification (async tag speed down nahi hone dega)
    # Check if already added to prevent duplicates
    if not soup.find('script', attrs={'src': 'https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-4780769968389227'}):
        adsense_script = soup.new_tag('script', attrs={
            'async': '',
            'src': 'https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-4780769968389227',
            'crossorigin': 'anonymous'
        })
        soup.head.append(adsense_script)
    ...
    *, ::before, ::after { box-sizing: border-box; border-width: 0; border-style: solid; border-color: #e5e7eb; }
    ...
    for post in grid_posts:
            ...
            card_html = f"""
            <a href="{FRONTEND_URL}/{post['slug']}" class="glass-list-item rounded-2xl overflow-hidden flex flex-row cursor-pointer group animate-fade-in text-inherit no-underline" style="min-height:148px;">
                <div class="flex-shrink-0" style="width:116px;min-height:148px;position:relative;overflow:hidden;">
                    <img src="{main_img}" loading="lazy" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;" class="group-hover:scale-110 transition duration-500" alt="{alt_text}">
                </div>
                <div class="flex flex-col justify-between flex-1 min-w-0 p-4 py-3">
                    <h3 class="font-bold text-green-900 leading-snug text-sm md:text-base line-clamp-3 group-hover:text-green-700 transition-colors">{post['title']}</h3>
                    <div>
                        <div class="flex items-center gap-2 text-xs text-green-700/80 mt-2 mb-1">
    ...
    # [NEW] PranavBlog Universal Analytics Tracker (Zero Render Blocking)
    analytics_js = """
    <script>
        setTimeout(function() {
            try {
                ...
                let vc = parseInt(localStorage.getItem('pranav_vc') || '0') + 1;
                localStorage.setItem('pranav_vc', vc);
                let siteHost = window.location.hostname;
                let pathSlug = window.location.pathname.replace(/^\/|\/$/g, '');
                let fullSlug = pathSlug === '' ? siteHost : siteHost + '/' + pathSlug;
                
                fetch('https://blog.pranavblog.online/track', {
                    method: 'POST',
    ...
    """
    soup.body.append(BeautifulSoup(analytics_js, 'html.parser'))
...
def background_task_runner():
    while True:
        ...
        time.sleep(300) # Sync aur HTML cache update har 5 minute me hoga
# --- Endpoints ---
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
    ...
    <url><loc>https://www.pranavblog.online/terms-of-use/</loc></url>
    <url><loc>https://www.pranavblog.online/about-us/</loc></url>
    <url><loc>https://www.pranavblog.online/search</loc></url>
</urlset>"""
    return Response(content=xml_content, media_type="application/xml")
# --- Search Endpoint ---
@app.get("/search", response_class=HTMLResponse)
async def serve_search_page():
    # Root (home) directory se search.html ka exact path nikalna
    base_dir = os.getcwd()
    search_path = os.path.join(base_dir, "search.html")
    
    # Agar file exist karti hai toh serve karo, warna 404 error
    if os.path.isfile(search_path):
        return FileResponse(search_path)
    raise HTTPException(status_code=404, detail="Search page not found")

# --- SEO & Redirect Logic (Universal Static File & Folder Router) ---
# DHYAN RAHE: Ye endpoint sabse aakhri me hona chahiye!
@app.get("/{full_path:path}", response_class=HTMLResponse)

async def catch_all_redirects(full_path: str):
    # 1. Ignore backend/API specific routes
    if full_path.startswith("api/") or full_path.startswith("img/") or full_path.startswith("cache/"):
        raise HTTPException(status_code=404, detail="Not Found")

    # FIX: Use __file__ to get the EXACT directory of this script on Cloud Servers!
    # FIX FOR RENDER: Use getcwd() as Render strictly runs from the root repo directory
    base_dir = os.getcwd()    
    # Trailing slash (/) hatana
    clean_path = full_path.strip("/")
    
    # 2. Security Check for Path Traversal (../)
    requested_path = os.path.abspath(os.path.join(base_dir, clean_path))
    if not requested_path.startswith(base_dir):
        raise HTTPException(status_code=403, detail="Forbidden")

    # 3. DIRECT FILE MATCH (E.g. /favicon.ico, /preview.jpg, /robots.txt)
    if os.path.isfile(requested_path):
        return FileResponse(requested_path)

    # 4. STRICT FOLDER ROUTING
    ...
    if first_segment in allowed_folders:
        folder_index = os.path.join(base_dir, first_segment, "index.html")
        if os.path.isfile(folder_index):
            return FileResponse(folder_index)
            
        # Failsafe: Agar galti se "about-us.html" root me pada ho instead of folder
        fallback_html = os.path.join(base_dir, f"{first_segment}.html")
        if os.path.isfile(fallback_html):
            return FileResponse(fallback_html)

    # 5. FALLBACK TO HOMEPAGE (404 ki jagah 200 OK for SEO)
    if os.path.exists(CACHED_HTML_PATH):
        with open(CACHED_HTML_PATH, "r", encoding="utf-8") as f:
            return HTMLResponse(content=f.read(), status_code=200)
            
    return HTMLResponse("<h1 style='text-align:center; padding-top:50px; color: #166534;'>Optimizing Site Engine... Please refresh.</h1>", status_code=200)
