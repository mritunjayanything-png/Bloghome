...
def generate_static_html():
    ...
    # [NEW FIX] Ab JS nahi, pre-compiled Tailwind Core CSS download karenge
    LOCAL_TAILWIND_CSS = os.path.join(CACHE_DIR, "tailwind-core.css")
    if not os.path.exists(LOCAL_TAILWIND_CSS):
        try:
            css_content = requests.get("https://cdnjs.cloudflare.com/ajax/libs/tailwindcss/2.2.19/tailwind.min.css").text
            with open(LOCAL_TAILWIND_CSS, "w", encoding="utf-8") as f:
                f.write(css_content)
        except Exception as e:
            print("Could not download Tailwind CSS", e)
    ...
    # [NEW FIX] 1-2 sec FOUC (Toota hua UI) rokne ke liye JS compiler hata rahe hain
    for script in soup.find_all('script'):
        if script.get('src') and 'tailwindcss.com' in script.get('src'):
            script.decompose() # Delete JS script permanently
            break
            
    # Core CSS link inject karna
    core_css_link = soup.new_tag('link', rel='stylesheet', href='/cache/tailwind-core.css')
    soup.head.append(core_css_link)

    # Inject Custom CSS specifically for arbitrary JIT classes used in your HTML
    custom_jit_css = r"""
    /* Custom Compiled Classes from Tailwind JIT */
    ...
    custom_style_tag.string = custom_jit_css
