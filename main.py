...
async def catch_all_redirects(full_path: str):
    ...
    # 2. PRIORITY 1: Direct File Check (For favicon.ico, preview.jpg, robots.txt etc.)
    # Agar root ya kisi subfolder me ye exact file match karti hai, toh serve karo.
    if os.path.isfile(requested_path):
        return FileResponse(requested_path)

    # 3. PRIORITY 2: Allowed Folders Check (Netlify redirect logic)
    allowed_folders = ["home", "about-us", "terms-of-use", "privacy-policy", "login", "admin", "generate", "search", "signup"]
    base_folder = full_path.split("/")[0] if full_path else ""
    
    if base_folder in allowed_folders:
        folder_index = os.path.join(base_dir, base_folder, "index.html")
        if os.path.isfile(folder_index):
            return FileResponse(folder_index)

    # 4. PRIORITY 3: Fallback to Homepage (404 ki jagah 200 for SEO)
    if os.path.exists(CACHED_HTML_PATH):
        with open(CACHED_HTML_PATH, "r", encoding="utf-8") as f:
