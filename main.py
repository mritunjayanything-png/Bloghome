...
def generate_static_html():
    ...
    if slider_track:
        ...
        for i, post in enumerate(slider_posts):
            ...
            main_img = f"/cache/images/{os.path.basename(db_path)}" if db_path else '/cache/images/default_feat.webp'
            ...
            dot_html = f'<button aria-label="Go to slide {i+1}" class="h-1.5 rounded-full transition-all duration-300 {active_class}" onclick="goToSlide({i})"></button>'
    ...
    for post in grid_posts:
            ...
            main_img = f"/cache/images/{os.path.basename(db_path)}" if db_path else '/cache/images/default_grid.webp'
            alt_text = get_alt_text(post['title'])
