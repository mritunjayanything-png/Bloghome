...
async def catch_all_redirects(full_path: str):
    # API, Image, ya Cache paths ko chhod dein
    if full_path.startswith("api/") or full_path.startswith("img/") or full_path.startswith("cache/"):
        raise HTTPException(status_code=404, detail="Not Found")

    # Security: Hacker ko server folder ke bahar (../) access karne se rokna
    base_dir = os.path.abspath(".")
    
    # Path ko clean karna (trailing slash hatana taaki routing perfect ho)
    ...
    # 1. DIRECT FILE MATCH (E.g. /favicon.ico, /preview.jpg, /about-us/index.html)
    # Agar ye exact file server pe exist karti hai, toh turant de do
    if os.path.isfile(requested_path):
        return FileResponse(requested_path)

    # 2. FOLDER INDEX MATCH (E.g. /privacy-policy -> /privacy-policy/index.html)
    # Agar user ne folder ka naam likha hai, toh check karo kya usme index.html hai?
    potential_index = os.path.join(requested_path, "index.html")
    if os.path.isfile(potential_index):
        return FileResponse(potential_index)

    # 3. FALLBACK TO HOMEPAGE (404 ki jagah 200 for SEO)
    # Agar na file mili, na folder me html mila, toh Homepage dikha do
    if os.path.exists(CACHED_HTML_PATH):
        with open(CACHED_HTML_PATH, "r", encoding="utf-8") as f:
